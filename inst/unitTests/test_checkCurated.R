test_checkCurated <- function(){
    ...
    template.file <- system.file("extdata/template_crc.csv", package = "curatedCRCData")
    ...
    dataset.names <- data(package="curatedCRCData")$results[, "Item"]
    ...
    curated <- lapply(dataset.names, function(x){
        data(list=x, package="curatedCRCData")
        pData(get(x))
    })
